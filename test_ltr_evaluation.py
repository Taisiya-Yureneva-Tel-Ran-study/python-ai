...
class Test(TestCase):
    def test_eval_ok(self):
        ...
        self.assertAlmostEqual(ltr.eval('( 10.75 - 3.93 ) * 2.0 - 13.4568 / 2.075'), 0.0882, places=3)
        expr = "(3 + (2 * 10 / (40 - 20))+(3 * 4)) * 10"
        self.assertAlmostEqual(160, ltr.eval(expr))
        expr = "-7 + 2 ** -4 / -2 + -8 * -3"
        self.assertAlmostEqual(24.0024, ltr.eval(expr))
        self.assertEqual(ltr.eval('0'), 0)
        self.assertEqual(ltr.eval('(3-23)-1 + 2'), -19)
        self.assertEqual(ltr.eval('(-3+23)/2'), 10)
